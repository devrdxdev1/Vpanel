...
class Database:
    ...
    def _create_tables(self):
        ...
        self._execute('''
            CREATE TABLE IF NOT EXISTS vps_group_assignments (
                ...
                FOREIGN KEY (group_id) REFERENCES vps_groups (id),
                FOREIGN KEY (vps_id) REFERENCES vps_instances (vps_id)
            )
        ''')
    ...
    def _initialize_settings(self):
        defaults = {
            ...
            'server_ip': SERVER_IP,
            'vps_hostname_prefix': VPS_HOSTNAME_PREFIX  # New setting
        }
    ...
    def get_banned_users(self):
        return [row[0] for row in self._fetchall('SELECT user_id FROM banned_users')]
...
def setup_container(container_id, memory, vps_id, ssh_port, root_password, watermark, welcome):
    try:
        ...
        success, _, stderr = run_docker_command(container_id, ["bash", "-c", f"echo 'root:{shlex.quote(root_password)}' | chpasswd"])
        if not success:
            raise Exception(f"Password set failed: {stderr}")
       
        cmd = f"echo '{shlex.quote(welcome)}' > /etc/motd && echo 'echo \"{shlex.quote(welcome)}\"' >> /root/.bashrc"
        ...
        hostname_cmd = f"echo '{prefix}{vps_id}' > /etc/hostname && hostname {prefix}{vps_id}"
        success, _, stderr = run_docker_command(container_id, ["bash", "-c", hostname_cmd])
        if not success:
            raise Exception(f"Hostname set failed: {stderr}")
       
        success, _, stderr = run_docker_command(container_id, ["bash", "-c", f"echo '{shlex.quote(watermark)}' > /etc/machine-info"])
        if not success:
    ...
    if request.method == 'POST':
        ...
        if user_data and check_password_hash(user_data['password'], password):
            if db.is_user_banned(user_data['id']):
                return render_template('login.html', error='Banned', panel_name=db.get_setting('panel_name', PANEL_NAME))
...
def register():
    ...
    if request.method == 'POST':
        ...
        if password != confirm or len(password) < 12:
            return render_template('register.html', error='Password mismatch or too short', panel_name=db.get_setting('panel_name', PANEL_NAME))
...
def profile():
    if request.method == 'POST':
        ...
        if not check_password_hash(user_data['password'], current) or new != confirm or len(new) < 12:
            return render_template('profile.html', error='Invalid password change', panel_name=db.get_setting('panel_name', PANEL_NAME), theme=current_user.theme)
...
def create_vps():
    ...
    if request.method == 'POST':
        try:
            ...
            user_id = current_user.id if not is_admin(current_user) else int(
                request.form.get('user_id', current_user.id)
            )

            if memory < 1 or memory > 512 or cpu < 1 or cpu > 32 or disk < 10 or disk > 1000:
...
def clone_vps(vps_id):
    token, vps = db.get_vps_by_id(vps_id)
    if not vps or vps['created_by'] != current_user.id:
        return jsonify({'error': 'Access denied'}), 403
...
def change_vps_password(vps_id):
    ...
    try:
        ...
        success, _, err = run_docker_command(vps['container_id'], ["bash", "-c", f"echo 'root:{shlex.quote(new_password)}' | chpasswd"])
        if not success:
...
def admin_panel():
    ...
    settings = {
        ...
        'vps_hostname_prefix': db.get_setting('vps_hostname_prefix', VPS_HOSTNAME_PREFIX)  # Added
    }
...
@admin_required
def admin_settings():
    for key in ['panel_name', 'watermark', 'welcome_message', 'server_ip', 'vps_hostname_prefix']:
...
__version__ = "2.0"  # Updated version
